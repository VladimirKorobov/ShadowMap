...
void Shape::CreateFlat(int width, int height)
{
	...
	v[0][width - 1] = 50;
	v[height - 1][0] = 50;
	v[height - 1][width - 1] = 50;


	v[height / 2][width / 2] = -150;

	float randfactor = width * 8.0e-4;
...
void Shape::Draw()
{
	for (size_t i = 0; i < chunks.size(); i++)
	{
		...
		//glDrawElements(chunks[i].type, 6, GL_UNSIGNED_INT, (GLvoid*)(sizeof(GLuint)*3)); GL_CHECK_ERRORS;
	}
}
