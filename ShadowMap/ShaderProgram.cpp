...
void ShaderProgram::SetUniform(const std::string &location, const glm::mat4 &value) const
{
  ...
  glUniformMatrix4fv(uniformLocation, 1, GL_FALSE, &value[0][0]); GL_CHECK_ERRORS;
}

void ShaderProgram::SetUniform(const std::string &location, const glm::mat3 &value) const
{
	GLint uniformLocation = glGetUniformLocation(shaderProgram, location.c_str()); GL_CHECK_ERRORS;
	if (uniformLocation == -1)
	{
	...
	glUniformMatrix3fv(uniformLocation, 1, GL_FALSE, &value[0][0]); GL_CHECK_ERRORS;
}
